...
const TABS = [
  {
    ...
    label: 'Oil',
    color: '#f59e0b',
    subtitle: 'WTI & Brent Crude · USD/bbl · Yahoo Finance (CL=F, BZ=F)',
  },
  {
    key: 'aluminum',
    label: 'Aluminum',
    color: '#94a3b8',
    subtitle: 'CME Aluminum Futures · USD/cwt · Yahoo Finance (ALI=F)',
  },
  {
    key: 'ss',
...
function ChartPanel({ title, data, lines, tabColor, useMonthlyFor }) {
    ...
    <div className="bg-[#1a1a1f] border border-[#2a2a32] rounded-xl p-5">
        ...
        <div className="flex flex-wrap gap-1">
          {tabs.map((t) => (
            <button
              ...
              onClick={() => setFilter(t)}
              className={`px-3 py-1 rounded text-xs font-medium transition-colors ${
                filter === t ? 'text-black' : 'bg-[#2a2a32] text-slate-300 hover:bg-[#3a3a44]'
              }`}
      ...
      {loading && (
        <div className="flex items-center justify-center py-10">
          <div className="w-5 h-5 border-2 border-t-transparent rounded-full animate-spin" style={{ borderColor: tabColor }} />
        </div>
      )}
          ...
          {filtered.map((item, i) => (
            <li key={i} className="py-3 flex flex-col gap-1">
              ...
              <div className="flex items-center gap-2 text-xs">
                {item.feed && (
                  <span className="px-1.5 py-0.5 rounded font-medium" style={{ color: tabColor, background: 'rgba(255,255,255,0.05)' }}>
                    {item.feed}
                  </span>
                )}
...
function OilTab({ tabColor }) {
  ...
  useEffect(() => {
    fetch('/api/commodity-data?commodity=oil')
      ...
      .then(({ instruments: inst, fetchedAt: fa }) => {
        setInstruments(inst || []);
        setFetchedAt(fa);
        setLoading(false);
      })
  ...
  const combinedDaily = useMemo(() => {
    if (!wti?.daily?.length || !brent?.daily?.length) return [];
    const wtiMap   = Object.fromEntries(wti.daily.map((d) => [d.date, d.close]));
    const brentMap = Object.fromEntries(brent.daily.map((d) => [d.date, d.close]));
    const allDates = [...new Set([...wti.daily.map((d) => d.date), ...brent.daily.map((d) => d.date)])].sort();
    return allDates.map((date) => ({ date, wti: wtiMap[date] ?? null, brent: brentMap[date] ?? null }));
  }, [wti, brent]);

  const combinedMonthly = useMemo(() => {
    if (!wti?.monthly?.length || !brent?.monthly?.length) return [];
    const wtiMap   = Object.fromEntries(wti.monthly.map((d) => [d.date, d.close]));
    const brentMap = Object.fromEntries(brent.monthly.map((d) => [d.date, d.close]));
    const allDates = [...new Set([...wti.monthly.map((d) => d.date), ...brent.monthly.map((d) => d.date)])].sort();
    return allDates.map((date) => ({ date, wti: wtiMap[date] ?? null, brent: brentMap[date] ?? null }));
  }, [wti, brent]);
  ...
  return (
    <div className="space-y-6">
        ...
        {brentKpi && (
          <>
            ...
            <KpiCard
              ...
              main={spread != null ? `$${fmt(Math.abs(spread))}` : '—'}
              sub={spread != null ? (spread > 0 ? 'Brent premium to WTI' : 'WTI premium to Brent') : ''}
              accent="#64748b"
            />
          </>
        )}
      ...
      <ChartPanel
        title="WTI & Brent Crude Oil (USD/bbl)"
        data={combinedDaily}
        lines={[
          { dataKey: 'wti',   name: 'WTI Crude',   color: wti?.color   || '#f59e0b' },
          { dataKey: 'brent', name: 'Brent Crude',  color: brent?.color || '#fb923c' },
        ]}
        tabColor={tabColor}
      ...
      <footer className="border-t border-[#2a2a32] pt-4 pb-2 text-xs text-slate-500 flex flex-wrap justify-between gap-2">
        <span>Source: Yahoo Finance (CL=F, BZ=F) · Front-month continuous futures · USD/bbl</span>
...
// ─── Tab: Single Instrument (Aluminum + SS) ───────────────────────────────────

function SingleTab({ commodity, tabColor, unit, footerSource, proxyNote }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard title={`${instrument.name} — Current`} main={`$${fmt(kpi.last?.close)}`} sub={fmtDate(kpi.last?.date)} accent={tabColor}>
          <ChangeChip value={kpi.dayChangePct} />
        </KpiCard>
        <KpiCard
          title="Day Change"
          main={kpi.dayChange != null ? `${kpi.dayChange >= 0 ? '+' : ''}$${fmt(Math.abs(kpi.dayChange))}` : '—'}
          sub={unit}
          accent={tabColor}
        >
          <ChangeChip value={kpi.dayChangePct} />
        ...
        <KpiCard
          ...
          sub={pctOf5YHigh != null ? `Current at ${fmt(pctOf5YHigh, 0)}% of 5Y high` : ''}
          accent="#64748b"
        />
      </div>

      <ChartPanel
        title={`${instrument.name} (${unit})`}
        data={instrument.daily}
        lines={[{ dataKey: 'close', name: instrument.name, color: tabColor }]}
        tabColor={tabColor}
        useMonthlyFor={instrument.monthly}
      />
  ...
  return (
    <div className="min-h-screen bg-[#0f0f11] text-white">
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        ...
        {/* Tab Nav */}
        <div className="flex gap-0 border-b border-[#2a2a32]">
          {TABS.map((t) => (
            <button
              key={t.key}
